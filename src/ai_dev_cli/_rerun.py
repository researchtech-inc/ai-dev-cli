...
from ai_dev_cli._state import hash_tracked_files, load_failure_dependency_hash, save_failure_dependency_hashes


@dataclass(frozen=True, slots=True)
class RerunSet:
    """Selected nodeids and whether pytest should run."""
...
def compute_rerun_set(repo_root: Path, scope_dirs: tuple[str, ...]) -> RerunSet:
    """Read lastfailed and testmon data, then return nodeids needing rerun."""
    ...
    if not testmon_path.exists():
        print(f"NOTE rerun-failed: testmon data unavailable; rerunning all {len(lastfailed)} failed entries")
        return RerunSet(tuple(lastfailed), should_run=True)

    try:
        with sqlite3.connect(testmon_path) as conn:
            nodeids = _filter_with_testmon(conn, lastfailed)
    except sqlite3.OperationalError:
        print(f"NOTE rerun-failed: testmon data unavailable; rerunning all {len(lastfailed)} failed entries")
        return RerunSet(tuple(lastfailed), should_run=True)
    except sqlite3.DatabaseError:
        print(
            "WARN rerun-failed: .testmondata corrupt; falling back to unfiltered rerun.",
            file=sys.stderr,
        )
        return RerunSet(tuple(lastfailed), should_run=True)
...
def _filter_with_testmon(conn: sqlite3.Connection, nodeids: tuple[str, ...]) -> list[str]:
    if not _has_testmon_schema(conn):
        print(f"NOTE rerun-failed: testmon data unavailable; rerunning all {len(nodeids)} failed entries")
        return list(nodeids)
    ...
    for nodeid in nodeids:
        ...
        if previous_hash is None or hash_tracked_files(*filenames) != previous_hash:
            selected.append(nodeid)
    return selected


def record_failure_dependency_hashes(repo_root: Path, nodeids: tuple[str, ...]) -> None:
    """Store current dependency digests for failed nodeids."""
    if not nodeids:
        return
